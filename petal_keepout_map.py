...
data_ptl = data_ptl[['device_loc', 'device_type', 'X', 'Y']]
data_flat = data_ptl.copy()
data_flat[['X', 'Y']] = trans.ptlXYZ_to_flatXY(data_ptl[['X', 'Y']].values.T).T
...
# petal map in flatXY
KEEPOUT_PTL_FLAT = trans.ptlXYZ_to_flatXY(KEEPOUT_PTL)
KEEPOUT_PTL_FLAT[:, 9] = [420, 250]
KEEPOUT_GFA_FLAT = trans.ptlXYZ_to_flatXY(KEEPOUT_GFA)
